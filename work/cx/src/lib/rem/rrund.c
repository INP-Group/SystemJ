...
{
    fprintf(stderr, "Usage: %s [-d] [-v] [port]\n", argv0);
...
static void Run(void)
{
  ...
  char               *args[2];
    ...
    while (1)
    {
        ...
        for (pctr = 0;  pctr < sizeof(path);  pctr++)
        {
            r = read(fd, path + pctr, 1);
            if (r != 1)
            {
        ...
        switch (r)
        {
            ...
            case -1:  /* Failed to fork */
                ...
                ReportProblem(fd, REMDRVP_RRUNDP, "%s::%s(): fork=%d: %s",
...
//        fprintf(stderr, "About to exec \n\"%s\"\n", path);
        ...
        ReportProblem(1, REMDRVP_RRUNDP, "%s::%s(): can't execv(\"%s\"): %s",
