...
static void UpdateRepers(fastadc_gui_t *gui)
{
  ...
  double  rpr_dsp[XH_PLOT_NUM_REPERS][FASTADC_MAX_LINES];
    ...
    for (nr = 0;  nr < XH_PLOT_NUM_REPERS;  nr++)
    {
        ...
        rpr_xs[nr] = is_r? atd->x2xs(gui->a.pfr.mes.info, x)
                         : rpr_xs[1] - rpr_xs[0];
...
static void ChnSwchKCB(Knob k __attribute__((unused)), double v, void *privptr)
{
...
static void MagnKCB(Knob k __attribute__((unused)), double v, void *privptr)
{
  pzframe_gui_dpl_t *pp    = privptr;
  fastadc_gui_t     *gui   = pp->p;
  int32              value = round(v);
...
static void InvpKCB(Knob k __attribute__((unused)), double v, void *privptr)
{
  pzframe_gui_dpl_t *pp    = privptr;
  fastadc_gui_t     *gui   = pp->p;
  int32              value = round(v);
...
static void CmprKCB(Knob k __attribute__((unused)), double v, void *privptr)
{
...
static void RprSwchKCB(Knob k __attribute__((unused)), double v, void *privptr)
{
  pzframe_gui_dpl_t *pp    = privptr;
  fastadc_gui_t     *gui   = pp->p;
  int32              value = round(v);
  ...
  Pixel      bgc;
    ...
    else if (kind == FASTADC_GUI_CTL_X_CMPR)
    {
        snprintf(spec, sizeof(spec),
                 " look=selector label='' widdepinfo='#+\v#T");
    ...
    else if (kind == FASTADC_GUI_CTL_LINE_LABEL)
    {
        ...
        SetKnobValue(k, gui->a.dcnv.lines[nl].show);
        SetKnobState(k, COLALARM_NONE);
    ...
    else if (kind == FASTADC_GUI_CTL_LINE_MAGN)
    {
        ...
        for (i = 0, sel_n = 0;  i < countof(fastadc_data_magn_factors);  i++)
        {
            sprintf(spec + strlen(spec), "%sx%d",
                    i == 0? "":"\v", fastadc_data_magn_factors[i]);
    ...
    {
        snprintf(spec, sizeof(spec),
                 " look=onoff label='^v' widdepinfo='panel'");
        prmp = gui->Line_prm + nl;
        prmp->p = gui;
        prmp->n = nl;
        k = CreateSimpleKnob(spec, 0, ABSTRZE(parent), InvpKCB, prmp);
        SetKnobValue(k, gui->a.dcnv.lines[nl].invp);
        return CNCRTZE(GetKnobWidget(k));
    }
    else if (kind == FASTADC_GUI_CTL_REPER_VALUE)
    {
        check_snprintf(spec, sizeof(spec), gui->a.dcnv.lines[nl].dpyfmt,
                       FastadcDataPvl2Dsp(&(gui->a), &(gui->a.mes), nl,
                                          (reprof_cxdtype(atd->ftd.dtype) == CXDTYPE_REPR_INT)
                                          ?
                                          FastadcDataRaw2Pvl(&(gui->a), &(gui->a.mes),
                                                             nl,
                                                             atd->range.int_r[1])
                                          :
                                                             atd->range.dbl_r[1]
    ...
    {
        sprintf(spec, "1:%d%s",
                atd->x2xs(gui->a.pfr.mes.info, atd->max_numpts - 1) + 1, atd->xs);
    ...
    else if (kind == FASTADC_GUI_CTL_REPER_ONOFF)
    {
        if (nr < FASTADC_GUI_2_REPERS)
        {
            ...
            SetKnobValue(gui->rpr_kswch[nr], gui->rpr_on[nr]);
        }
        else
        {
                           ...
                           fastadc_gui_mkctls_t mkctls)
{
  ...
  int       in_toolbar;
    ...
    if (cpanel != NULL  &&  mkctls != NULL)
    {
        ...
        adc_win = XhWindowOf(ABSTRZE(ctls));
        in_toolbar  = 0;
        leds_parent = ABSTRZE(gui->g.local_leds);
        if (leds_parent == NULL)
        {
            in_toolbar  = 1;
            leds_parent = XhGetWindowAlarmleds(adc_win);
        }
...
static void UpdateBG(pzframe_gui_t *inherited_gui)
{
  ...
  XhPixel bg = XhGetColor((gui->look.black? XH_COLOR_BGRAPH_BG : XH_COLOR_GRAPH_BG));
                    ...
                    int  info_changed)
{
  fastadc_gui_t *gui = (fastadc_gui_t *)inherited_gui;
    ...
    FastadcGuiRenewPlot(gui, gui->a.pfr.mes.info[gui->a.atd->param_numpts], info_changed);
}
...
static double      gui_raw2pvl(void *privptr, int nl, int raw)
{
  fastadc_gui_t *gui = privptr;

    return FastadcDataRaw2Pvl(&(gui->a),
                              &(gui->a.mes),
                              nl, raw);
}
...
static double      gui_pvl2dsp(void *privptr, int nl, double pvl)
{
  fastadc_gui_t *gui = privptr;
    ...
    return FastadcDataPvl2Dsp(&(gui->a),
                              &(gui->a.mes),
                              nl, pvl);
}
...
static int         gui_x2xs   (void *privptr, int x)
{
  fastadc_gui_t *gui = privptr;
    ...
    return gui->a.atd->x2xs(gui->a.pfr.mes.info, x);
}
...
static void        set_rpr_to(fastadc_gui_t *gui, int nr, int x)
{
    ...
    if (gui->rpr_kswch[nr]) SetKnobValue(gui->rpr_kswch[nr], gui->rpr_on[nr]);
    UpdateRepers(gui);
}
                            ...
                            cda_serverid_t def_regsid, int base_chan)
{
  ...
  int         nl;
    ...
    if (gui->g.look.nocontrols) cpanel_loc = XH_PLOT_CPANEL_ABSENT;

    gui->plot       = XhCreatePlot(parent, 
                                   XH_PLOT_Y_OF_X, 
                                   gui->a.atd->num_lines,
                                   gui, 
                                   gui_raw2pvl, gui_pvl2dsp, gui_x2xs, gui_xs, 
                                   gui->d->plotdraw != NULL ? gui->d->plotdraw
                                                            : gui_draw,
                                   gui_set_rpr,
                                   gui->look.width, gui->look.height,
                                   cpanel_loc, gui->look.black,
                                   0);
