...
static int GET_RRQ_USECS(ottcam_privrec_t *me)
{
  int  ret;
    ...
    ret = me->pz.cur_args[OTTCAM_PARAM_RRQ_MSECS] * 1000;
                          ...
                          void *privptr)
{
  ...
  OTTCAM_DATUM_T   *p;
    ...
    memcpy(me->pz.cur_args, me->pz.nxt_args, sizeof(me->pz.cur_args));
  
    me->pz.cur_args[OTTCAM_PARAM_X]    = 0;
    me->pz.cur_args[OTTCAM_PARAM_Y]    = 0;
    me->pz.cur_args[OTTCAM_PARAM_W]    = OTTCAM_MAX_W;
    me->pz.cur_args[OTTCAM_PARAM_H]    = OTTCAM_MAX_H;
    me->pz.cur_args[OTTCAM_PARAM_MISS] = 0;
    ...
    for (y = 0;  y < SIMSQ_H;  y++)
        for (x = 0,  p = me->retdata + (me->sim_y + y) * OTTCAM_MAX_W + me->sim_x;
             x < SIMSQ_W;
             x++,    p++)
            *p = (me->pz.cur_args[OTTCAM_PARAM_T] +
                  me->pz.cur_args[OTTCAM_PARAM_K] +
                  x * 4 + y * 4) & 1023;

    ReturnBigc(devid, 0, me->pz.cur_args, OTTCAM_NUM_PARAMS,
...
static int32 ValidateParam(pzframe_drv_t *pdr, int n, int32 v)
{
  ottcam_privrec_t *me = PDR2ME(pdr);
    ...
    else if (n == OTTCAM_PARAM_T)
    {
        if (v < 0)     v = 0;
        if (v > 65535) v = 65535;
    }
    else if (n == OTTCAM_PARAM_SYNC)
    {
        v = v != 0;
    }
    //!!! X,Y,W,H
...
{   
  ottcam_privrec_t *me = PDR2ME(pdr);
    ...
    me->pz.nxt_args[OTTCAM_PARAM_X]         = 0;
    me->pz.nxt_args[OTTCAM_PARAM_Y]         = 0;
    me->pz.nxt_args[OTTCAM_PARAM_W]         = OTTCAM_MAX_W;
    me->pz.nxt_args[OTTCAM_PARAM_H]         = OTTCAM_MAX_H;
    me->pz.nxt_args[OTTCAM_PARAM_K]         = 16;
    me->pz.nxt_args[OTTCAM_PARAM_T]         = 30*100;
    me->pz.nxt_args[OTTCAM_PARAM_SYNC]      = 0;
    me->pz.nxt_args[OTTCAM_PARAM_RRQ_MSECS] = RRQ_USECS / 1000;
}
...
static int  StartMeasurements(pzframe_drv_t *pdr)
{
  ottcam_privrec_t *me = PDR2ME(pdr);
    ...
    /* */
    me->pz.retdataunits = OTTCAM_DATAUNITS;
    me->pz.retdatasize  =
        me->pz.cur_args[OTTCAM_PARAM_W] *
        me->pz.cur_args[OTTCAM_PARAM_H] *
        me->pz.retdataunits;

    /* */
    me->curframe++;
    me->state     = STATE_INITIAL;
    me->rqd_h     = me->pz.cur_args[OTTCAM_PARAM_H];
    me->rest      = me->rqd_h;
    me->smth_rcvd = 0;
    ...
    enq_pkt(me, OTTCMD_WRITEREG,  0x07, REG_0x07_VAL(me->pz.cur_args[OTTCAM_PARAM_SYNC]));
    enq_pkt(me, OTTCMD_WRITEREG,  0x0B, me->pz.cur_args[OTTCAM_PARAM_T]);
    enq_pkt(me, OTTCMD_WRITEREG,  0x0D, 0);
    enq_pkt(me, OTTCMD_WRITEREG,  0x35, me->pz.cur_args[OTTCAM_PARAM_K]);
    ...
    enq_pkt(me, OTTCMD_SET_FSIZE, me->pz.cur_args[OTTCAM_PARAM_W],
                                  me->pz.cur_args[OTTCAM_PARAM_H]);
...
static rflags_t ReadMeasurements(pzframe_drv_t *pdr)
{
  ...
  int               l;
    ...
    me->pz.cur_args[OTTCAM_PARAM_MISS] = me->rest;
        ...
        switch (hdr.id)
        {
            ...
            case OTTRPY_DATA:
                ...
                if (me->is_rcvd[line_n]                       ||
                    line_n < me->pz.cur_args[OTTCAM_PARAM_Y]  ||
                    line_n > me->pz.cur_args[OTTCAM_PARAM_Y] +
                             me->pz.cur_args[OTTCAM_PARAM_H] - 1)
                {
                    goto CONTINUE_TO_NEXT_PACKET;
                }
  ...
  int                 r;
    ...
    else
    {
        ...
        pzframe_drv_init(&(me->pz), devid,
                         OTTCAM_NUM_PARAMS,
                         OTTCAM_PARAM_SHOT, OTTCAM_PARAM_ISTART,
                         OTTCAM_PARAM_WAITTIME, OTTCAM_PARAM_STOP,
                         OTTCAM_PARAM_ELAPSED,
                         ValidateParam,
...
static void ottcam_rw_p  (int devid, void *devptr, int firstchan, int count, int32 *values, int action)
{
  ottcam_privrec_t *me = (ottcam_privrec_t *)devptr;
...
static void ottcam_bigc_p(int devid, void *devptr,
...
static CxsdMainInfoRec ottcam_main_info[] =
{
    {OTTCAM_NUM_PARAMS, 1},
};

static CxsdBigcInfoRec ottcam_bigc_info[] =
{
    {1, 
        OTTCAM_NUM_PARAMS, 0,                                          0,
        OTTCAM_NUM_PARAMS, OTTCAM_MAX_W*OTTCAM_MAX_H*OTTCAM_DATAUNITS, OTTCAM_DATAUNITS},
};

DEFINE_DRIVER(ottcam,  "Ottmar's Camera",
              NULL, NULL,
              sizeof(ottcam_privrec_t), NULL,
              0, 0,
              NULL, 0,
              countof(ottcam_main_info), ottcam_main_info,
              countof(ottcam_bigc_info), ottcam_bigc_info,
              ottcam_init_d, ottcam_term_d,
              ottcam_rw_p, ottcam_bigc_p);
