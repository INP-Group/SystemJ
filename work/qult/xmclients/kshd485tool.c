...
const char *app_name      = "kshd485tool";
const char *app_class     = "KShD485Tool";
const char *app_title     = "KShD485 test app";
...
#define COLORLED_LINE(name, comnt, dtype, n) \
    {name, NULL, NULL, NULL, NULL, comnt, LOGT_READ, dtype, LOGK_DIRECT, LOGC_NORMAL, PHYS_ID(KSHD485_BASE + KSHD485_C_STATUS_BASE + n)}
#define BUT_LINE(name, title, n) \
    ...
    GLOBELEM_START("ctl", "Direct control", 3, 1)
        SUBELEM_START("info", "Info", 2, 2)
            {"devver", "Dev.ver.", NULL, "%4.0f", "withlabel", NULL, LOGT_READ, LOGD_TEXT, LOGK_DIRECT, LOGC_NORMAL, PHYS_ID(KSHD485_BASE + KSHD485_C_DEV_VERSION)},
            {"devsrl", "serial#",  NULL, "%3.0f", "withlabel", NULL, LOGT_READ, LOGD_TEXT, LOGK_DIRECT, LOGC_NORMAL, PHYS_ID(KSHD485_BASE + KSHD485_C_DEV_SERIAL)},
        SUBELEM_END("noshadow,notitle,nocoltitles,norowtitles", NULL),
        SUBELEM_START("config", "Config", 5, 1)
            SUBELEM_START("curr", "Current", 2, 2)
                {"work", "Work", "", "", "#T0A\v0.2A\v0.3A\v0.5A\v0.6A\v1.0A\v2.0A\v3.5A", NULL, LOGT_WRITE1, LOGD_SELECTOR, LOGK_DIRECT, LOGC_NORMAL, PHYS_ID(KSHD485_BASE + KSHD485_C_WORK_CURRENT)},
                {"hold", "Hold", "", "", "#T0A\v0.2A\v0.3A\v0.5A\v0.6A\v1.0A\v2.0A\v3.5A", NULL, LOGT_WRITE1, LOGD_SELECTOR, LOGK_DIRECT, LOGC_NORMAL, PHYS_ID(KSHD485_BASE + KSHD485_C_HOLD_CURRENT)},
            SUBELEM_END("noshadow,notitle,nocoltitles,norowtitles", NULL),
            {"hold_delay", "Hold dly", "1/30s", "%3.0f", "withunits", NULL, LOGT_WRITE1, LOGD_TEXT, LOGK_DIRECT, LOGC_NORMAL, PHYS_ID(KSHD485_BASE + KSHD485_C_HOLD_DELAY), .minnorm=0, .maxnorm=255, .placement="horz=right"},
            {"cfg_byte", "Cfg byte", NULL, "%3.0f", NULL, NULL, LOGT_WRITE1, LOGD_TEXT, LOGK_DIRECT, LOGC_NORMAL, PHYS_ID(KSHD485_BASE + KSHD485_C_CONFIG_BYTE)},
            SUBELEM_START("vel", "Velocity", 2, 2)
                {"min", "[",   "/s",   "%5.0f", "withlabel", NULL, LOGT_WRITE1, LOGD_TEXT, LOGK_DIRECT, LOGC_NORMAL, PHYS_ID(KSHD485_BASE + KSHD485_C_MIN_VELOCITY), .minnorm=32, .maxnorm=12000},
                {"max", "-",   "]/s",   "%5.0f", "withlabel,withunits", NULL, LOGT_WRITE1, LOGD_TEXT, LOGK_DIRECT, LOGC_NORMAL, PHYS_ID(KSHD485_BASE + KSHD485_C_MAX_VELOCITY), .minnorm=32, .maxnorm=12000},
            SUBELEM_END("noshadow,notitle,nocoltitles,norowtitles", NULL),
            {"accel",   "Accel", "/s^2", "%5.0f", "withunits", NULL, LOGT_WRITE1, LOGD_TEXT, LOGK_DIRECT, LOGC_NORMAL, PHYS_ID(KSHD485_BASE + KSHD485_C_ACCELERATION), .minnorm=32, .maxnorm=65535, .placement="horz=center"},
        ...
        SUBELEM_START("operate", "Operation", 2, 1)
            ...
            SUBELEM_START("steps", "Steps", 6, 3)
                {"numsteps", NULL, NULL, "%8.0f", NULL, NULL, LOGT_WRITE1, LOGD_TEXT, LOGK_DIRECT, LOGC_NORMAL, PHYS_ID(KSHD485_BASE + KSHD485_C_NUM_STEPS)},
                BUT_LINE("go",         "GO",          KSHD485_BASE + KSHD485_C_GO),
#if 0
                BUT_LINE("go_unaccel", "GO unaccel.", KSHD485_BASE + KSHD485_C_GO_WO_ACCEL),
#else
                {"stopcond", "StopC", NULL, "%3.0f", "withlabel", NULL, LOGT_WRITE1, LOGD_TEXTND, LOGK_DIRECT, LOGC_NORMAL, PHYS_ID(KSHD485_BASE + KSHD485_C_GO_WO_ACCEL)},
#endif
                EMPTY_CELL(),
                BUT_LINE("stop",       "STOP",             KSHD485_BASE + KSHD485_C_STOP),
                BUT_LINE("switchoff",  "Switch OFF",       KSHD485_BASE + KSHD485_C_SWITCHOFF),
            SUBELEM_END("noshadow,notitle,nocoltitles,norowtitles", NULL),
    ...
    GLOBELEM_START("cycling", "Cycling", 4, 1)
        ...
        {"stop", "Stop", NULL, NULL, NULL, NULL, LOGT_WRITE1, LOGD_BUTTON, LOGK_CALCED, LOGC_NORMAL, PHYS_ID(-1),
            (excmd_t[]){
                CMD_GETP_I(KSHD485_BASE + KSHD485_C_STOP), CMD_POP,
