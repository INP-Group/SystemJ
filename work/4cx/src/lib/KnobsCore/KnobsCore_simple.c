...
static void Simple_SetPhys_m(DataKnob k, double v)
{
...
dataknob_cont_vmt_t Simple_VMT = {.SetPhys = Simple_SetPhys_m};
                           ...
                           void            *privptr)  // Pointer to pass to cb
{
  ...
  CxKnobParam_t *params = NULL;
    ...
    k->u.k.is_rw = rec.is_rw;

    if (rec.is_local) k->u.k.behaviour |= DATAKNOB_B_NO_WASJUSTSET;
...
void      SetSimpleKnobValue(DataKnob k, double v)
{
  ...
  rflags_t              rflags;
    ...
    if (
        !kd->is_rw  ||
        (
         difftime(timenow, kd->usertime) > KNOBS_USERINACTIVITYLIMIT  &&
         !kd->wasjustset
        )
       )
