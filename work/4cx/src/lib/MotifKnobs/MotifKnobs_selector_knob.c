...
static void HandleBgIdxChange(DataKnob k, int newidx)
{
  ...
  int  oldidx;
    ...
    if (newidx != oldidx) SelectorColorize_m(k, k->u.k.curstate);
}
...
static int CreateSelectorKnob(DataKnob k, CxWidget parent)
{
  ...
  Dimension              mrg_hgt;
    ...
    k->u.k.behaviour |= DATAKNOB_B_IS_SELECTOR | DATAKNOB_B_STEP_FXD;
    if (me->ign_otherop) k->u.k.behaviour |= DATAKNOB_B_IGN_OTHEROP;
    ...
    XtSetArg(al[ac], XmNtraversalOn, k->u.k.is_rw); ac++;
    ...
    selector = XmCreateOptionMenu(CNCRTZE(parent),
                                  k->u.k.is_rw? "selector_i" : "selector_o",
                                  al, ac);
...
static void SelectorSetValue_m(DataKnob k, double v)
{
  ...
  int                 item = (int)v;
    ...
    if (item < 0)             item = 0;
    if (item >= me->numitems) item = me->numitems;
...
dataknob_knob_vmt_t motifknobs_selector_vmt =
{
    {DATAKNOB_KNOB, "selector",
        ...
        0,
        CreateSelectorKnob, DestroySelectorKnob, NULL},
    SelectorSetValue_m, SelectorColorize_m, NULL
};
