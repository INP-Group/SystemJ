...
static GC AllocPixGC(Widget w, XhPixel pix)
{
  XtGCMask   mask;  
  XGCValues  vals;
...
void XhViewportSetCpanelState(Xh_viewport_t *vp, int state)
{
    ...
    if (state == 0)
    {
        ...
        XtUnmanageChild(vp->btn_cpanel_off);
        XtManageChild  (vp->btn_cpanel_on);
    }
    else
    {
        XtManageChild  (vp->cpanel);
        XtManageChild  (vp->btn_cpanel_off);
        XtUnmanageChild(vp->btn_cpanel_on);
    }
}

void XhViewportSetBG     (Xh_viewport_t *vp, XhPixel bg)
{
  GC  new_bkgdGC;
