                    ...
                    XtPointer callData  __attribute__((unused)))
{
  XhCommandProc  proc = (XhCommandProc)(window->commandproc);
    ...
    if (window->sendcmdonclose  &&  proc != NULL)
    {
        proc(window, 0);
        return;
    }
