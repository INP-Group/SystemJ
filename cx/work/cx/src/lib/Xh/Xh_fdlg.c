...
static void OkCancelCB(Widget     w,
                       XtPointer  closure,
                      ...
                      const char *pattern, int cmd)
{
  ...
  XmString  ok_s      = NULL;
    ...
    XtAddCallback(dialog, XmNokCallback,     OkCancelCB, lint2ptr(cmd));
    XtAddCallback(dialog, XmNcancelCallback, OkCancelCB, lint2ptr(cmd + 1));
  ...
  loaddlg_rec_t *privrec;
    ...
    if (flst_cmd != 0)
    {
        w = XtVaCreateManagedWidget("showFiles", xmPushButtonWidgetClass,
                                    dialog,
                                    NULL);
        XtAddCallback(w, XmNactivateCallback, OkCancelCB, lint2ptr(flst_cmd));
    }
