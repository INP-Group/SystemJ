...
static void DrawPlotView  (plot_info_t *plot)
{
  ...
  char       rns;
    ...
    for (nr = 0;  nr < XH_PLOT_NUM_REPERS;  nr++)
        if (plot->rpr_on[nr])
        {
            x = (plot->rpr_at[nr] / plot->x_cmpr) - plot->vprt.horz_offset;
            if (x >= 0  &&  x < grf_w)
            {
    ...
    for (i = 0;  i < limit;  i += 10)
    {
        ...
        if ((i % 100) == 0)
        {
            snprintf(buf, sizeof(buf), "%d%s",
                     plot->x2xs(plot->privptr, i * plot->x_cmpr),
                           ...
                           int bn, int x, int y)
{
  ...
  int          nr;
    ...
    {
        plot->mouse_x = (x + vp->horz_offset) * plot->x_cmpr;
...
XhPlot    XhCreatePlot(Widget parent, int type, int maxplots,
                       ...
                       XhPlot_plotdraw_t  draw_one,
                       XhPlot_set_rpr_t   set_rpr,
                       int init_w, int init_h, int cpanel_loc, int black,
                       ...)
{
  plot_info_t *plot;
  ...
  int          nl;
    ...
    parts = 
        ...
        XH_VIEWPORT_CPANEL
        ;
    if      (cpanel_loc == XH_PLOT_CPANEL_AT_LEFT)   parts |= XH_VIEWPORT_CPANEL_AT_LEFT;
    else if (cpanel_loc == XH_PLOT_CPANEL_AT_RIGHT)  parts |= XH_VIEWPORT_CPANEL_AT_RIGHT;
    else if (cpanel_loc == XH_PLOT_CPANEL_AT_TOP)    parts |= XH_VIEWPORT_CPANEL_AT_TOP;
    else if (cpanel_loc == XH_PLOT_CPANEL_AT_BOTTOM) parts |= XH_VIEWPORT_CPANEL_AT_BOTTOM;
...
int       XhPlotSetMaxPts  (XhPlot plot, int maxpts)
{
    ...
    plot->maxpts            = maxpts;
    plot->vprt.horz_maximum = maxpts / plot->x_cmpr;
...
void      XhPlotSetCmpr    (XhPlot plot, int cmpr)
{
    ...
    plot->x_cmpr = cmpr;
    plot->vprt.horz_maximum = plot->maxpts / plot->x_cmpr;
...
void      XhPlotOneDraw (XhPlot plot, plotdata_t *data, int magn, GC gc)
{
  ...
  do_flush_t  do_flush;
    ...
    if (data == NULL) return;
    p8  = (uint8 *)  (data->x_buf) + plot->vprt.horz_offset * plot->x_cmpr;
    p16 = (int16 *)  (data->x_buf) + plot->vprt.horz_offset * plot->x_cmpr;
    p32 = (int32 *)  (data->x_buf) + plot->vprt.horz_offset * plot->x_cmpr;
    f32 = (float32 *)(data->x_buf) + plot->vprt.horz_offset * plot->x_cmpr;
    f64 = (float64 *)(data->x_buf) + plot->vprt.horz_offset * plot->x_cmpr;

    for (npoints = 0,                               x_ctr = 0,
         i = plot->vprt.horz_offset * plot->x_cmpr, x = 0;

         i < data->numpts                       &&  x < grf_w;
...
void      XhPlotSetBG   (XhPlot plot, XhPixel bg)
{
