...
/*
  Note: for DGRAM connections a special layout of _sysbuf is used --
        each datagram is prefixed with 32-bit length.
...
typedef struct
{
    ...
    size_t         _sysbufused;    // Amount of data in it
...
static int StreamReadyForWrite(fdinfo_t *fr)
{
  int  r;
    ...
    if (fr->_sysbufused == 0)
    {
        reporterror("%s(handle=%d,fd=%d): READY-FOR_WRITE, but _sysbufused==0!\n",
...
static int StreamReadyForRead (fdinfo_t *fr)
{
 ...
 FALLTHROUGH:
        ...
        if (count != 0)
        {
            r = uintr_read(fr->fd, fr->reqbuf + fr->reqreadsize, count);
            errs = IsReadError(fr, r);
            ////if (errs != 0) fprintf(stderr, "1{%d} errs=%d/%d repcount=%d\n", handle, errs, errno, repcount);
            if (errs < 0) return -1; // Unrecoverable errors -- return -1
...
static int StreamSend        (fdinfo_t *fr, uint8 *buf, size_t size)
{
  int        r;
  size_t     newbufsize;
    ...
    if (size != 0)
    {
        ...
        fast_memcpy(fr->_sysbuf + fr->_sysbufused, buf, size);
...
static int DgramSend         (fdinfo_t *fr, uint8 *buf, size_t size)
{
    return -1;
}
...
static int StringSend         (fdinfo_t *fr, uint8 *buf, size_t size)
{
                               ...
                               size_t         len_add)
{
  ...
  struct  sigaction  act;
    ...
    if (fdtype == FDIO_STRING  || fdtype == FDIO_STRING_CONN)
...
int  fdio_send                (fdio_handle_t handle, void *buf, size_t size)
{
  fdinfo_t  *fr;
