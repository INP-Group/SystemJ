...
/* Uninterruptible I/O */
int n_write(int fd, const void *buf, size_t count);
int n_read (int fd, void       *buf, size_t count);
 ...
 *
 *      unitr_read(fd) doesn't block if is called after select() which
...
static inline int uintr_read(int s, void *buf, size_t count)
{
  int  r;
...
static inline int uintr_write(int s, void *buf, size_t count)
{
  int  r;
