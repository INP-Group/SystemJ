...
enum
{
    XCAC208_MAX_NSTEPS     = 29,
    XCAC208_MAX_STEP_COUNT = 65535, // ==655.35s
...
typedef struct
{
    ...
    int32            ch_cfg[XCAC208_CONFIG_CHAN_COUNT]; // Cache of config channels
    int32            ch_cur[XCAC208_CHAN_OUT_n_COUNT]; // Current value
    int32            ch_rcv[XCAC208_CHAN_OUT_n_COUNT]; // Current value was received

    int32            ch_spd[XCAC208_CHAN_OUT_n_COUNT]; // Speed
    int32            ch_trg[XCAC208_CHAN_OUT_n_COUNT]; // Target
    
    int8             ch_lkd[XCAC208_CHAN_OUT_n_COUNT]; // Is locked by table
    int8             ch_isc[XCAC208_CHAN_OUT_n_COUNT]; // Is changing now
    int8             ch_nxt[XCAC208_CHAN_OUT_n_COUNT]; // Next write may be performed (previous was acknowledged)
    int8             ch_fst[XCAC208_CHAN_OUT_n_COUNT]; // This is the FirST step
    int8             ch_fin[XCAC208_CHAN_OUT_n_COUNT]; // This is a FINal write

    int32            t_nsteps[XCAC208_CHAN_OUT_n_COUNT];
    int              t_aff_c [XCAC208_CHAN_OUT_n_COUNT];
    int32            t_times [XCAC208_CHAN_OUT_n_COUNT][XCAC208_MAX_NSTEPS];
    int32            t_vals  [XCAC208_CHAN_OUT_n_COUNT][XCAC208_MAX_NSTEPS];
    int              t_mode;
    uint8            t_file[XCAC208_MAX_NSTEPS * (2 + 4*XCAC208_CHAN_OUT_n_COUNT)];
    size_t           t_file_bytes;
} privrec_t;

static psp_paramdescr_t xcac208_params[] =
{
    PSP_P_INT ("beg",  privrec_t, ch_beg,   0,                          0, XCAC208_CHAN_ADC_n_COUNT-1),
    PSP_P_INT ("end",  privrec_t, ch_end,   XCAC208_CHAN_ADC_n_COUNT-1, 0, XCAC208_CHAN_ADC_n_COUNT-1),
...
#define ADC_MODE_IS_NORM() \
    (me->ch_cfg[XCAC208_CHAN_ADC_MODE] == XCAC208_ADC_MODE_NORM)


/*=== ZZZ ===*/

static int8 UNSUPPORTED_chans[XCAC208_CONFIG_CHAN_COUNT] =
{
    [XCAC208_CHAN_RESERVED6]    = 1,
    [XCAC208_CHAN_RESERVED7]    = 1,
    [XCAC208_CHAN_RESERVED8]    = 1,
    [XCAC208_CHAN_DO_CALIBRATE] = 1,
    [XCAC208_CHAN_DIGCORR_MODE] = 1,
    [XCAC208_CHAN_DIGCORR_V]    = 1,
    [XCAC208_CHAN_RESERVED19]   = 1,
};
...
{
    ReturnChanGroup(me->devid,   XCAC208_CHAN_DO_TAB_ACTIVATE, 1,
                    me->ch_cfg + XCAC208_CHAN_DO_TAB_ACTIVATE,
                    &invl_rflags);
}
...
{
  static int32 table_ctls[][XCAC208_CHAN_DO_TAB_CNT] =
  {
  ...
  int          x;
    ...
    me->t_mode = mode;
    memcpy(me->ch_cfg + XCAC208_CHAN_DO_TAB_BASE,
           &(table_ctls[mode]), sizeof(table_ctls[mode]));

    for (x = 0;  x < XCAC208_CHAN_DO_TAB_CNT;  x++)
        ReturnCtlCh(me, XCAC208_CHAN_DO_TAB_BASE + x);

    me->ch_cfg[XCAC208_CHAN_OUT_MODE] = (mode < TMODE_LOAD? XCAC208_OUT_MODE_NORM
                                                          : XCAC208_OUT_MODE_TABLE);
    ReturnCtlCh(me, XCAC208_CHAN_OUT_MODE);
}
...
static void SendMULTICHAN(privrec_t *me)
{
  canqelem_t  item;
    ...
    if (!me->fast  ||
        me->ch_cfg[XCAC208_CHAN_ADC_BEG] != me->ch_cfg[XCAC208_CHAN_ADC_END])
    {
        item.datasize = 6;
        item.data[0]  = DESC_MULTICHAN;
        item.data[1]  = me->ch_cfg[XCAC208_CHAN_ADC_BEG];
        item.data[2]  = me->ch_cfg[XCAC208_CHAN_ADC_END];
        item.data[3]  = me->ch_cfg[XCAC208_CHAN_ADC_TIMECODE];
        item.data[4]  = PackMode(me->ch_cfg[XCAC208_CHAN_ADC_GAIN],
                                 me->ch_cfg[XCAC208_CHAN_ADC_GAIN]);
        item.data[5] = 0; // Label
        DoDriverLog(me->devid, 0,
                    "%s:MULTICHAN beg=%d end=%d time=%d gain=%d", __FUNCTION__,
                    me->ch_cfg[XCAC208_CHAN_ADC_BEG],
                    me->ch_cfg[XCAC208_CHAN_ADC_END],
                    me->ch_cfg[XCAC208_CHAN_ADC_TIMECODE],
                    me->ch_cfg[XCAC208_CHAN_ADC_GAIN]);
    }
    else
    {
        ...
        item.data[1]  = me->ch_cfg[XCAC208_CHAN_ADC_BEG] |
                       (me->ch_cfg[XCAC208_CHAN_ADC_GAIN] << 6);
        item.data[2]  = me->ch_cfg[XCAC208_CHAN_ADC_TIMECODE];
...
static void DoSoftReset(privrec_t *me, int flags)
{
  int  x;
    ...
    {
        me->ch_cfg[XCAC208_CHAN_ADC_BEG]      = me->ch_beg;
        me->ch_cfg[XCAC208_CHAN_ADC_END]      = me->ch_end;
        me->ch_cfg[XCAC208_CHAN_ADC_TIMECODE] = me->timecode;
        me->ch_cfg[XCAC208_CHAN_ADC_GAIN]     = me->gain;
    }
    
    /*!!! bzero() something? ch_nxt[]:=1? */

    if (flags & DSRF_RETCONFIGS)
        for (x = 0;  x < XCAC208_CONFIG_CHAN_COUNT;  x++)
            ReturnCtlCh(me, x);

    if (flags & DSRF_CUR2OUT)
        for (x = 0;  x < XCAC208_CHAN_OUT_n_COUNT;  x++)
            ReturnChanGroup(me->devid, XCAC208_CHAN_OUT_n_BASE + x, 1,
                            me->ch_cur + x,
                            &zero_rflags);
}
...
static void SetToStopped(privrec_t *me)
{
  ...
  uint8      data[8];
    ...
    for (l = 0;  l < XCAC208_CHAN_OUT_n_COUNT;  l++)
...
static void xcac208_rst(int devid, void *devptr, int is_a_reset);
static void xcac208_in (int devid, void *devptr,
                        int desc, int datasize, uint8 *data);
static void xcac208_alv(int devid, void *devptr,
                        sl_tid_t tid,
                        void *privptr);
static void xcac208_hbt(int devid, void *devptr,
                        sl_tid_t tid,
                        void *privptr);

static int xcac208_init_d(int devid, void *devptr, 
                          ...
                          const char *auxinfo)
{
  ...
  int        l;       // Line #
    ...
    for (l = 1;  l < XCAC208_CHAN_OUT_n_COUNT;  l++)
    ...
    me->handle = me->iface->add(devid, devptr,
                                ...
                                DEVTYPE,
                                xcac208_rst, xcac208_in,
                                XCAC208_NUMCHANS * 2 +
                                    (XCAC208_MAX_NSTEPS * (2 + 4*XCAC208_CHAN_OUT_n_COUNT) + 7-1) / 7 * 2 /* For table*/,
                                XCAC208_CHAN_REGS_BASE);
    if (me->handle < 0) return me->handle;

    DoSoftReset(me, DSRF_RESET_CHANS);
    SetTmode   (me, TMODE_NONE);
    
    sl_enq_tout_after(devid, devptr, ALIVE_USECS,     xcac208_alv, NULL);
    sl_enq_tout_after(devid, devptr, HEARTBEAT_USECS, xcac208_hbt, NULL);

    return DEVSTATE_OPERATING;
}

static void xcac208_term_d(int devid, void *devptr __attribute__((unused)))
{
  privrec_t *me    = (privrec_t *) devptr;
  uint8      data[8];
...
static void xcac208_rst(int   devid    __attribute__((unused)),
                        void *devptr,
                        int   is_a_reset)
{
  privrec_t  *me     = (privrec_t *) devptr;
...
static void xcac208_in (int devid, void *devptr __attribute__((unused)),
                        int desc, int datasize, uint8 *data)
{
  privrec_t  *me       = (privrec_t *) devptr;
  int         l;       // Line #
  ...
  int         gain_code;
    ...
    switch (desc)
    {
        ...
        case DESC_READONECHAN:
            ...
            if (l < 0  ||  l >= XCAC208_CHAN_ADC_n_COUNT) return;
            ReturnChanGroup(devid,
                            XCAC208_CHAN_ADC_n_BASE + l,
                            1, &val, &rflags);
            break;

        case DESC_RD_DAC_n_BASE ... DESC_RD_DAC_n_BASE + XCAC208_CHAN_OUT_n_COUNT-1:
            ...
            ReturnChanGroup(devid,
                            XCAC208_CHAN_OUT_CUR_n_BASE + l,
                            1, me->ch_cur + l, &rflags);
            if (!me->ch_isc[l]  ||  !SHOW_SET_IMMED)
                ReturnChanGroup(devid,
                                XCAC208_CHAN_OUT_n_BASE + l,
                                1, me->ch_cur + l, &rflags);
            else if (me->ch_fst[l])
            {
                me->ch_fst[l] = 0;
...
static void xcac208_rw_p(int devid, void *devptr, int firstchan, int count, int *values, int action)
{
  privrec_t  *me     = (privrec_t *) devptr;
  ...
  uint8       data[8];
    ...
    for (n = 0;  n < count;  n++)
    {
        ...
        if      (x >= XCAC208_CHAN_REGS_BASE  &&  x <= XCAC208_CHAN_REGS_LAST)
            me->iface->regs_rw(me->handle, action, x, values + n);
        else if (x >= XCAC208_CHAN_REGS_RSVD_B  &&  x <= XCAC208_CHAN_REGS_RSVD_E)
        {
            // Non-existent register channels...
        }
        else if (
                 ( // Is it a config channel?
                  x >= XCAC208_CONFIG_CHAN_BASE  &&
                  x <  XCAC208_CONFIG_CHAN_BASE + XCAC208_CONFIG_CHAN_COUNT
                 )
                 &&
                 (
        ...
        else if (x == XCAC208_CHAN_ADC_BEG)
        {
            if (ADC_MODE_IS_NORM())
            {
                if (val < 0)
                    val = 0;
                if (val > XCAC208_CHAN_ADC_n_COUNT - 1)
                    val = XCAC208_CHAN_ADC_n_COUNT - 1;
                if (val > me->ch_cfg[XCAC208_CHAN_ADC_END])
                    val = me->ch_cfg[XCAC208_CHAN_ADC_END];
                me->ch_cfg[x] = val;
                SendMULTICHAN(me);
            }
            ReturnCtlCh(me, x);
        }
        else if (x == XCAC208_CHAN_ADC_END)
        {
            if (ADC_MODE_IS_NORM())
            {
                if (val < 0)
                ...
                if (val > XCAC208_CHAN_ADC_n_COUNT - 1)
                    val = XCAC208_CHAN_ADC_n_COUNT - 1;
                if (val < me->ch_cfg[XCAC208_CHAN_ADC_BEG])
                    val = me->ch_cfg[XCAC208_CHAN_ADC_BEG];
                me->ch_cfg[x] = val;
                SendMULTICHAN(me);
            }
            ReturnCtlCh(me, x);
        }
        else if (x == XCAC208_CHAN_ADC_TIMECODE)
        {
            if (ADC_MODE_IS_NORM())
            {
                if (val < MIN_TIMECODE)
        ...
        else if (x == XCAC208_CHAN_ADC_GAIN)
        {
            if (ADC_MODE_IS_NORM())
            {
                if (val < MIN_GAIN)
        ...
        else if (x == XCAC208_CHAN_DO_RESET)
        {
        ...
        else if (x == XCAC208_CHAN_ADC_MODE  ||  x == XCAC208_CHAN_OUT_MODE)
        {
            /* These two are in fact read channels */
            ReturnCtlCh(me, x);
        }
        /* Process table-related channels similarly */
        else if (x == XCAC208_CHAN_DO_TAB_DROP      ||
                 x == XCAC208_CHAN_DO_TAB_ACTIVATE  ||
                 x == XCAC208_CHAN_DO_TAB_START     ||
                 x == XCAC208_CHAN_DO_TAB_STOP      ||
                 x == XCAC208_CHAN_DO_TAB_PAUSE     ||
                 x == XCAC208_CHAN_DO_TAB_RESUME)
        {
            if (action == DRVA_WRITE        &&
                val    == CX_VALUE_COMMAND  &&
                (me->ch_cfg[x] & CX_VALUE_DISABLED_MASK) == 0)
            {
                if      (x == XCAC208_CHAN_DO_TAB_DROP)
                {
                ...
                else if (x == XCAC208_CHAN_DO_TAB_ACTIVATE)
                {
                  ...
                  uint32  f_accls[XCAC208_CHAN_OUT_n_COUNT][XCAC208_MAX_NSTEPS];
                  uint32  f_incrs[XCAC208_CHAN_OUT_n_COUNT][XCAC208_MAX_NSTEPS];
                  ...
                  canqelem_t  isep;
                    ...
                    for (fc = -1, l = XCAC208_CHAN_OUT_n_COUNT-1;  l >= 0;  l--)
                    {
                            ...
                            for (tr = 1;  tr < me->t_nsteps[fc];  tr++)
                            {
                                if (me->t_times[l][tr] != me->t_times[fc][tr])
                                {
                                    DoDriverLog(devid, 0 /*!!! Or what?*/,
                                                ...
                                                l, tr, me->t_times[l][tr]*XCAC208_USECS_IN_STEP, fc, tr, me->t_times[fc][tr]*XCAC208_USECS_IN_STEP);
                ...
                else if (x == XCAC208_CHAN_DO_TAB_START)
                {
                ...
                else if (x == XCAC208_CHAN_DO_TAB_STOP)
                {
                ...
                else if (x == XCAC208_CHAN_DO_TAB_PAUSE)
                {
                ...
                else if (x == XCAC208_CHAN_DO_TAB_RESUME)
                {
                    SetTmode(me, TMODE_RUNN);
     ...
     /* else if(x ... XCAC208_CHAN_ADC_n_BASE...) isn't required since those are returned upon measurement */
        else if (x >= XCAC208_CHAN_OUT_n_BASE  &&
                 x <  XCAC208_CHAN_OUT_n_BASE + XCAC208_CHAN_OUT_n_COUNT)
        {
            l = x - XCAC208_CHAN_OUT_n_BASE;
        ...
        else if (x >= XCAC208_CHAN_OUT_RATE_n_BASE  &&
                 x <  XCAC208_CHAN_OUT_RATE_n_BASE + XCAC208_CHAN_OUT_n_COUNT)
        {
            l = x - XCAC208_CHAN_OUT_RATE_n_BASE;
            if (action == DRVA_WRITE)
            {
        ...
        else if (x >= XCAC208_CHAN_OUT_CUR_n_BASE  &&
                 x <  XCAC208_CHAN_OUT_CUR_n_BASE + XCAC208_CHAN_OUT_n_COUNT)
        {
            l = x - XCAC208_CHAN_OUT_CUR_n_BASE;
            if (me->ch_rcv[l])
...
static void xcac208_bigc_p(int devid, void *devptr,
...
#define RETDATACOUNT(nsteps, nchans) (nsteps * (1 + nchans))
  ...
  int32       retinfo[1];
  int32       retdata[RETDATACOUNT(XCAC208_MAX_NSTEPS, XCAC208_CHAN_OUT_n_COUNT)];
  size_t      retdatasize;
...
#define CHECK_IF(cond, format, args...)                                     \
    ...
    CHECK_IF(chan < 0  ||  chan >= XCAC208_NUM_BIGCS, "bigc=%d is unknown", chan);
    CHECK_IF(nargs != 0  &&  nargs     != 1, "nargs=%d, !=1",     nargs);
    CHECK_IF(nargs != 0  &&  dataunits != 4, "dataunits=%zu, !=4", dataunits);
    nsteps = 0;
    if (nargs != 0) nsteps = args[0];
    CHECK_IF(nargs != 0  &&  (nsteps < 1  ||  nsteps > XCAC208_MAX_NSTEPS),
             "nsteps=%d, out_of[1...%d]", nsteps, XCAC208_MAX_NSTEPS);
    
    if (chan >= XCAC208_BIGC_ITAB_n_BASE  &&
        chan <  XCAC208_BIGC_ITAB_n_BASE + XCAC208_CHAN_OUT_n_COUNT)
    {
        l = chan - XCAC208_BIGC_ITAB_n_BASE;
        ...
        if (nargs != 0  ||  0)
        {
            ...
            t_aff_c[l] = abs(table[1]) < XCAC208_VAL_DISABLE_TABLE_CHAN;
            
            /* Now, walk through rows */
            for (tr = 0;  tr < nsteps;  tr++)
            {
                step_msecs = table[tr * (1 + 1) + 0];
                step_csecs = step_msecs / XCAC208_USECS_IN_STEP;
                /* Check validity of time. */
                if (tr != 0)
                    CHECK_IF(step_csecs < 1  ||  step_csecs > XCAC208_MAX_STEP_COUNT,
                             "time[#%d]=%d, out_of[%d...%d]",
                             tr, step_msecs, XCAC208_USECS_IN_STEP, XCAC208_MAX_STEP_COUNT * XCAC208_USECS_IN_STEP);
                else
                    step_csecs = 0;
                ...
                if (val > MAX_ALWD_VAL) val = MAX_ALWD_VAL;
                if (val < MIN_ALWD_VAL) val = MIN_ALWD_VAL;
                if (!t_aff_c[l]) val = XCAC208_VAL_DISABLE_TABLE_CHAN;
                t_vals[l][tr] = val;
            }

            /* Pad unspecified data */
            for (tr = nsteps;  tr < XCAC208_MAX_NSTEPS;  tr++)
            {
                t_times[l][tr] = 0;
                t_vals [l][tr] = XCAC208_VAL_DISABLE_TABLE_CHAN;
            }
        ...
        for (tr = 0;  tr < nsteps;  tr++)
        {
            retdata[tr * (1 + 1) + 0] = me->t_times[l][tr] * XCAC208_USECS_IN_STEP;
            retdata[tr * (1 + 1) + 1] = me->t_vals [l][tr];
        }
    }
    else /*if (chan == XCAC208_BIGC_TAB_WHOLE)*/
    {
        /* Treat nargs==0 as "read" */
        if (nargs != 0  ||  0)
        {
            rqd_datasize = RETDATACOUNT(nsteps, XCAC208_CHAN_OUT_n_COUNT) * 4;
            CHECK_IF(datasize != rqd_datasize,
                     "datasize=%zu, !=%zu=nsteps*(1+%d)*4",
                     datasize, rqd_datasize, XCAC208_CHAN_OUT_n_COUNT);
    
            /* Determine which channels are affected and which aren't */
            for (l = 0;  l < XCAC208_CHAN_OUT_n_COUNT;  l++)
                t_aff_c[l] = abs(table[1 + l]) < XCAC208_VAL_DISABLE_TABLE_CHAN;
    
            /* Now, walk through rows... */
            for (tr = 0;  tr < nsteps;  tr++)
            {
                base = tr * (1 + XCAC208_CHAN_OUT_n_COUNT);
        ...
        retinfo[0] = nsteps;
        retdatasize = RETDATACOUNT(nsteps, XCAC208_CHAN_OUT_n_COUNT) * 4;
        for (tr = 0;  tr < nsteps;  tr++)
        {
            base = tr * (1 + XCAC208_CHAN_OUT_n_COUNT);
            retdata[base + 0] = me->t_times[fc][tr] * XCAC208_USECS_IN_STEP;
...
static CxsdMainInfoRec xcac208_main_info[] =
{
    {XCAC208_CONFIG_CHAN_COUNT,  1}, // Params and table control
    CANKOZ_REGS_MAIN_INFO,
    {XCAC208_CHAN_REGS_RSVD_E-XCAC208_CHAN_REGS_RSVD_B+1, 0},
    {XCAC208_CHAN_ADC_n_COUNT,   0}, // ADC_n
    {XCAC208_CHAN_OUT_n_COUNT*2, 1}, // OUT_n, OUT_RATE_n
    {XCAC208_CHAN_OUT_n_COUNT,   0}, // OUT_CUR_n
};

static CxsdBigcInfoRec xcac208_bigc_info[] =
{
    // Individual tables
    {XCAC208_CHAN_OUT_n_COUNT,
        1, sizeof(int32)*XCAC208_MAX_NSTEPS*2, sizeof(int32),
        1, sizeof(int32)*XCAC208_MAX_NSTEPS*2, sizeof(int32)},
    // Whole-channels table
    {1,
        1, XCAC208_CHAN_OUT_n_COUNT*sizeof(int32)*XCAC208_MAX_NSTEPS*2, sizeof(int32),
        1, XCAC208_CHAN_OUT_n_COUNT*sizeof(int32)*XCAC208_MAX_NSTEPS*2, sizeof(int32)},
};

DEFINE_DRIVER(xcac208, "eXtended CAC208 support",
              NULL, NULL,
              sizeof(privrec_t), xcac208_params,
              2, 3,
              NULL, 0,
              countof(xcac208_main_info), xcac208_main_info,
              countof(xcac208_bigc_info)*0-1, xcac208_bigc_info,
              xcac208_init_d, xcac208_term_d,
              xcac208_rw_p, xcac208_bigc_p);
