#include <X11/Intrinsic.h>
#include <Xm/Xm.h>
#include <Xm/Form.h>
#include <Xm/Label.h>
#include <Xm/Separator.h>


#include "vcamimg_data.h"
#include "vcamimg_gui.h"
#include "vcamimg_cpanel_decor.h"

#include "tsycamv_data.h"
#include "tsycamv_gui.h"

#include "drv_i/tsycamv_drv_i.h"


static Widget tsycamv_mkctls(pzframe_gui_t           *gui,
                             vcamimg_type_dscr_t     *atd,
                             Widget                   parent,
                             pzframe_gui_mkstdctl_t   mkstdctl,
                             pzframe_gui_mkparknob_t  mkparknob)
{
  vcamimg_gui_t *vcamimg_gui = pzframe2vcamimg_gui(gui);

  Widget  cform;    // Controls form
  Widget  line1;
  Widget  line2;

  Widget  w1;
  Widget  w2;

    /* 0. General layout */
    /* A container form */
    cform = XtVaCreateManagedWidget("form", xmFormWidgetClass, parent,
                                    XmNshadowThickness, 0,
                                    NULL);
    /* Two lines */
    line1 = XtVaCreateManagedWidget("form", xmFormWidgetClass, cform,
                                    XmNshadowThickness, 0,
                                    NULL);
    line2 = XtVaCreateManagedWidget("form", xmFormWidgetClass, cform,
                                    XmNshadowThickness, 0,
                                    NULL);
    attachtop(line2, line1, CHL_INTERKNOB_V_SPACING);

    /* 1. Line 1: standard controls */
    /* A "commons" */
    w1 = mkstdctl(gui, line1, VCAMIMG_GUI_CTL_COMMONS, 0, 0);

    w2 = mkstdctl(gui, line1, VCAMIMG_GUI_CTL_DPYMODE, 0, 0);
    attachleft(w2, w1, CHL_INTERKNOB_H_SPACING);
    w1 = w2;

    w2 = mkstdctl(gui, line1, VCAMIMG_GUI_CTL_NORMALIZE, 0, 0);
    attachleft(w2, w1, CHL_INTERKNOB_H_SPACING);
    w1 = w2;

    w2 = mkstdctl(gui, line1, VCAMIMG_GUI_CTL_MAX_RED, 0, 0);
    attachleft(w2, w1, CHL_INTERKNOB_H_SPACING);
    w1 = w2;

    w2 = mkstdctl(gui, line1, VCAMIMG_GUI_CTL_0_VIOLET, 0, 0);
    attachleft(w2, w1, CHL_INTERKNOB_H_SPACING);
    w1 = w2;

    /* 1. Line 2: parameters */
    w1 = mkparknob(gui, line2,
                   " look=button label='Stop'",
                   TSYCAMV_PARAM_STOP);

    w2 = mkparknob(gui, line2,
                   " look=text label='Ampl=' widdepinfo='withlabel'"
                   " minnorm=0 maxnorm=255 step=1 dpyfmt='%3.0f'",
                   TSYCAMV_PARAM_K);
    attachleft(w2, w1, CHL_INTERKNOB_H_SPACING + 40);
    w1 = w2;

    w2 = mkparknob(gui, line2,
                   " look=text label='Expo=' widdepinfo='withlabel'"
                   " minnorm=0 maxnorm=511 step=30 dpyfmt='%5.0f'",
                   TSYCAMV_PARAM_T);
    attachleft(w2, w1, CHL_INTERKNOB_H_SPACING);
    w1 = w2;

    w2 = mkparknob(gui, line2,
                   " look=choicebs label=''"
                   " widdepinfo='#TImmed\f\flit=amber\vSync\f\flit=white'",
                   TSYCAMV_PARAM_SYNC);
    attachleft(w2, w1, CHL_INTERKNOB_H_SPACING);
    w1 = w2;

    w2 = mkparknob(gui, line2,
                   " ro look=text label='Miss:' widdepinfo='withlabel'"
                   " dpyfmt='%-3.0f'",
                   TSYCAMV_PARAM_MISS);
    attachleft(w2, w1, CHL_INTERKNOB_H_SPACING);
    w1 = w2;

    return cform;
}

pzframe_gui_dscr_t *tsycamv_get_gui_dscr(void)
{
  static vcamimg_gui_dscr_t  dscr;
  static int                 dscr_inited;

    if (!dscr_inited)
    {
        VcamimgGuiFillStdDscr(&dscr,
                              pzframe2vcamimg_type_dscr(tsycamv_get_type_dscr()));

        dscr.cpanel_loc = 0/*!!!BOTTOM!!!*/;
        dscr.mkctls     = tsycamv_mkctls;

        dscr_inited = 1;
    }
    return &(dscr.gkd);
}
