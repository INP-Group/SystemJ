...
enum
{
    ...
    CANKOZ_KEEPALIVE_TIMEOUT = 60 * 1000 * 1000, // 1 m
};
...
typedef struct
{
    ...
    int             devid;
    void           *devptr;
    int             devcode;  // !!! Should change to devcodes[10], init them to -1, and
  ...
  char *errstr;
    ...
    if (r != CANHAL_OK  &&  r != lp->last_wr_r)
    {
        ...
        DoDriverLog(my_lyrid, 0 | errflg, "%s(%d:k%d/%d, dlc=%d, cmd=0x%02x)%s",
                    __FUNCTION__, lp-lines, kid, prio, dlc, 
...
static int  cankoz_get_dev_ver(int handle, int *hw_ver_p, int *sw_ver_p)
{
  int           line;
  int           kid;
  kozdevinfo_t *dp;
...
static void cankoz_regs_rw (int handle, int action, int chan, int32 *wvp)
{
  ...
  uint8         mask;
    ...
    if (n < 0  ||  n >= CANKOZ_IOREGS_CHANCOUNT) return;
...
{
  int32      regsaddrs [CANKOZ_IOREGS_CHANCOUNT];
                             ...
                             void *privptr)
{
  ...
  int           is_a_reset;
    ...
    /**/
    dp     = lp->devs + kid;
    handle = encode_handle(lp-lines, kid);
