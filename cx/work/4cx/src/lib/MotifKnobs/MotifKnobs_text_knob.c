...
{
    PSP_P_FLAG("withlabel", text_privrec_t, has_label,  1, 0),
    PSP_P_FLAG("nolabel",   text_privrec_t, has_label,  0, 1),
    
    PSP_P_FLAG("noincdec",  text_privrec_t, no_incdec,  1, 0),
    PSP_P_FLAG("nounits",   text_privrec_t, no_units,   1, 0),
    
    PSP_P_FLAG("groupable", text_privrec_t, is_gpbl,    1, 0),
    
    PSP_P_FLAG("compact",   text_privrec_t, is_compact, 1, 0),
...
{
  text_privrec_t *me = (text_privrec_t *)k->privptr;
  ...
  XmFontList      lfl;
    ...
    if (me->is_gpbl) k->u.k.behaviour |= DATAKNOB_B_IS_GROUPABLE;
    if ((k->u.k.behaviour & DATAKNOB_B_IS_GROUPABLE) != 0  &&  k->u.k.is_rw)
        me->is_gpbl   = 1;
    ...
    if (me->is_gpbl)
    {
        k->u.k.behaviour |= DATAKNOB_B_IS_GROUPABLE;
    ...
    /* Text... */
    me->text = (k->u.k.is_rw? MotifKnobs_CreateTextInput
...
static void TextColorize_m(DataKnob k, knobstate_t newstate)
{
  text_privrec_t *me = (text_privrec_t *)k->privptr;
  Pixel           fg;
  Pixel           bg;
...
static void TextPropsChg_m(DataKnob k, DataKnob old_k  __attribute__((unused)))
{
  text_privrec_t *me = (text_privrec_t *)k->privptr;
    ...
    if (k->u.k.behaviour & DATAKNOB_B_IS_GROUPABLE  &&
        me->grpmrk != NULL                          &&
...
dataknob_knob_vmt_t motifknobs_text_vmt =
{
    {DATAKNOB_KNOB, "text",
        sizeof(text_privrec_t), text2textopts,
        0,
        CreateNormalText, MotifKnobs_CommonDestroy_m, NULL},
    TextSetValue_m, TextColorize_m, TextPropsChg_m
};

dataknob_knob_vmt_t motifknobs_defknob_vmt =
{
    {DATAKNOB_KNOB, DEFAULT_KNOB_LOOK_NAME,
        sizeof(text_privrec_t), NULL,
        0,
        CreateDefKnob, MotifKnobs_CommonDestroy_m, NULL},
    TextSetValue_m, TextColorize_m, TextPropsChg_m
};
