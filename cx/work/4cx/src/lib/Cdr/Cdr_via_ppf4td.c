...
#define KPD_LINE(name, field, TYPE) \
    {name, -offsetof(kparamsdescr_t, field), __CX_CONCATENATE(TYPE,_fparser)}
                          ...
                          DataKnob uplink)
{
  ...
  int             pn;
    ...
    {
        k->u.k.kind      = kind;
        k->u.k.is_rw     = (k->u.k.kind == DATAKNOB_KIND_WRITE);
        k->u.k.behaviour = behaviour;
    }
    k->uplink = uplink;
    ...
    if (type == DATAKNOB_KNOB)
    {
        k->u.k.curstate = KNOBSTATE_JUSTCREATED;
