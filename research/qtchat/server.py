...
class ServerDlg(QPushButton):
    ...
    def authMsg(self, s, text):
        ...
        if text in self.users:
            stream << QString('AUTH') << QString(
                text +
                ' is taken,please change one.')
        else:
            self.users[text] = s
            stream << QString('CHAT') << QString(
                text +
                ',Great,now you can chat!')
        stream.device().seek(0)
        stream.writeUInt32(reply.size() - SIZEOF_UINT32)
        s.write(reply)
    ...
    def sendMessage(self, text, socketId):
        ...
        for user in self.users:
            if self.users[user].socketDescriptor() == socketId:
                message = '<p>' + str(now.strftime('%Y-%m-%d %H:%M:%S')) + \
                    '</p>' + '<font color=red>You</font> > {}'.format(text)
            else:
                message = '<p>' + str(now.strftime('%Y-%m-%d %H:%M:%S')) + '</p>' + \
                    '<font color=blue>' + sender + \
                    '</font>' + ' > {}'.format(text)
            reply = QByteArray()
            stream = QDataStream(reply, QIODevice.WriteOnly)
            stream.setVersion(QDataStream.Qt_4_2)
